...
The existing publisher-controlled release configuration produced TestFlight **1.0 (3)** from `a82a4ad`; Apple validation, upload and processing succeeded. Tracked development identity/signing remain unchanged. Delivery is currently limited to TestFlight; public App Store publication is not authorized. Finalized public privacy policy, metadata/screenshots and the remaining native runtime checks are still required before public submission. See [App Store and privacy preparation](docs/APP_STORE.md) for current beta status and release boundaries; repository checks cannot guarantee approval.
