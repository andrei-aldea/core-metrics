...
Unsigned builds validate compilation. For interactive development, select `Core Metrics` / `My Mac` in Xcode and Run using a local signing configuration. The app appears in the menu bar, not the Dock. Click its live value to select statistics, choose a text mode, open About or Settings, or Quit. Metric Help explains the readings. Copy Current Readings copies full selected names and current values, including readings shortened in the status label. Launch at Login is optional and off until you enable it in Settings; macOS manages its registration.
