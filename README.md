...
For UI changes, verify the running status item, panel and Settings, immediate preference updates, keyboard focus, VoiceOver, light/dark appearances, accessibility display settings, large text, and constrained menu-bar space. The final F7khvi desktop run passed all eight UI scenarios, including native outside-click dismissal, three full names, seven Compact readings and saved-selection startup, Settings recovery and stable live geometry. Debug/Release builds, 68 unit-test declarations with 161 executions, analysis, configuration and source/packaged artifact checks also passed, as did the separate ad-hoc-signed Release build. Known AppIntents build warnings and the observed beta-runtime layout diagnostic remain documented limitations. The wider crowded/notched display and accessibility matrix remains release work. See the [development runbook](docs/DEVELOPMENT.md) for exact workflows and limitations.
