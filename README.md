...
For UI changes, verify the running status item, panel and Settings, immediate preference updates, keyboard focus, VoiceOver, light/dark appearances, accessibility display settings, large text, and constrained menu-bar space. All seven NativeVerified UI scenarios passed on the reviewed desktop, including three full names, seven Compact readings and saved-selection startup, Settings recovery and stable live geometry. The deterministic fixture measured the same 52-point-wide frame for six alternating 9%/100% publications. The final default validator also passed Debug/Release builds, 66 unit-test declarations with 142 executions, analysis and artifact checks, with no unexpected warnings. The wider crowded/notched display and accessibility matrix remains release work. See the [development runbook](docs/DEVELOPMENT.md) for exact workflows and limitations.
...
AppKit owns an `NSStatusItem` with an explicit positive width derived from the selected stats, mode and locale. Live samples update its native attributed button title without recomputing that width. A public `NSPopover` hosts the SwiftUI selection panel; Settings remains a native SwiftUI scene registered through `NSHostingSceneRepresentation`. The status item and Settings preview share the same attributed title: label prefixes and static separators use the normal system font, while padded values use its monospaced counterpart at the same system size (13 points on the reviewed runtime). Width measures static text in its actual font and adds conservative locale-aware value columns. Settings explicitly converts each AppKit font run into a SwiftUI font. Percentage columns use the locale's complete 100% representation with a four-character minimum in every mode; byte columns reserve eight characters. Compact places one space between stats and the other modes use two. There is no bitmap renderer or artificial width cap. This gives the app explicit allocation control in response to the reported value-dependent movement; the final local UI scenarios verified geometry and seven-stat Compact visibility on this display. That result does not guarantee space on every desktop. See [ADR-019](docs/DECISIONS.md#adr-019--own-native-status-item-geometry-and-lifetime).
