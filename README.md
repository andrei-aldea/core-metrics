...
For UI changes, verify the running status item, panel and Settings, immediate preference updates, keyboard focus, VoiceOver, light/dark appearances, accessibility display settings, large text, and constrained menu-bar space. The final RViaos validation passed all seven UI scenarios on the reviewed desktop, including three full names, seven Compact readings and saved-selection startup, Settings recovery and stable live geometry. The deterministic fixture measured the same 58-point-wide frame and position for six alternating 9%/100% publications. Debug/Release builds, 68 unit-test declarations with 161 executions, analysis and artifact checks also passed. Units and UI tests had no failures or skips. Known AppIntents build warnings and the observed beta-runtime layout diagnostic remain documented limitations. The wider crowded/notched display and accessibility matrix remains release work. See the [development runbook](docs/DEVELOPMENT.md) for exact workflows and limitations.
