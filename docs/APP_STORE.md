...
**Current check — September 22, 2026:** The new distribution-signed **1.0 (3)** candidate from `a82a4ad` passed Apple validation, uploaded successfully and completed processing. Following the user's TestFlight-only instruction, it is assigned to **External Testers** and now **Testing**. The requested tester is confirmed **Accepted**; the group has one tester and automatic notification is enabled for build 3. No public invitation link was created. The completed processing follow-up remains paused. An initial file-permission rejection was corrected by rebuilding with normal artifact permissions; final archive/export signature, sandbox, hardened-runtime, privacy and readability checks pass. The repository now guards against the same permission defect. Fresh validation passes Debug/Release builds, all 71 unit tests and analysis. See the [tester delivery and validation evidence](PROJECT_ANALYSIS_REPORT.md#testflight-tester-delivery-and-validation-repair--september-22-2026).

Safari confirmed that the earlier **1.0 (2)** is **Approved** for external beta testing and that the beta review contact/feedback fields are saved. The public version remains **Prepare for Submission**, with no screenshots or selected build, empty copyright and public App Review contact fields, and automatic release selected. Its description includes all four modes; its saved review notes still contain the obsolete single-selection restriction for Values Only. UI automation has recovered: the panel geometry repair passes all 71 unit and 13 desktop tests in final RC validation `1XS43u`. It is newer than uploaded build 3 and needs a new TestFlight candidate. Public submission/publication is explicitly excluded by the user; the remaining public-launch checklist is deferred. Older observations below remain historical where superseded.
