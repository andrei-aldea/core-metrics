...
**Current check — September 22, 2026:** The panel geometry repair is committed and pushed as **`51218f1`**. Its distribution-signed **1.0 (4)** archive and package pass identity/version, signatures, sandbox, hardened runtime, privacy, Release isolation and file-permission checks. Apple validation succeeded after a transient gateway retry. The subsequent upload ended with **exit 70 / Apple HTTP 504 gateway timeout** during analysis acknowledgement; a refreshed Core Metrics TestFlight page still showed only builds 1–3. A later user-requested retry also ended with exit 70 and HTTP 504 during Apple’s upload-checksum update; build 4 remained absent after another portal refresh. Build 4 is not claimed uploaded or available. The preserved candidate will be retried by **Finish Core Metrics TestFlight build** every 30 minutes, checking for an existing/active upload first and remaining quiet during unchanged service errors.
...
Safari confirmed that the earlier **1.0 (2)** is **Approved** for external beta testing and that the beta review contact/feedback fields are saved. The public version remains **Prepare for Submission**, with no screenshots or selected build, empty copyright and public App Review contact fields, and automatic release selected. Its description includes all four modes; its saved review notes still contain the obsolete single-selection restriction for Values Only. UI automation has recovered: the panel geometry repair passes all 71 unit and 13 desktop tests in final RC validation `1XS43u`. It is included in the preserved build-4 candidate awaiting successful upload. Public submission/publication is explicitly excluded by the user; the remaining public-launch checklist is deferred. Older observations below remain historical where superseded.
...
| --- | --- |
| Source and installed app | Geometry repair `51218f1` is pushed and is the source of candidate 1.0 (4). TestFlight build 3 remains the available prior candidate. No installed-app replacement is claimed by this delivery. |
...
| Shipping identity | Development retains the neutral `org.example.CoreMetrics` identifier. Private release overrides reuse the authorized existing publisher identity; the build-4 archive/export checks pass. No tracked bundle identifier, team, certificate, entitlement or deployment target changed. |
...
| Final validation | Final RC validation `1XS43u` passes Debug/Release, 71 unit tests, all 13 desktop tests, analysis, packaged checks, sandbox and bundle permissions. Five known AppIntents warnings remain; there are no other build warnings. Real login, sleep/wake and the broader accessibility/display matrix remain separate. |
...
For distribution archive, export, Apple validation and upload commands, use normal build-file permissions (`umask 022`). Keep private release configuration and logs inside an owner-only parent directory instead of applying `umask 077` to Xcode's build process. Verify that files in both the archived app and expanded installer payload are readable by non-root users, and that directories and executables retain traversal/execution permissions. On September 22, Apple validation rejected an otherwise correctly signed package because one archived app file lacked non-owner read access; rebuilding with normal permissions resolved the local packaging defect. Signature verification alone does not detect this problem.
