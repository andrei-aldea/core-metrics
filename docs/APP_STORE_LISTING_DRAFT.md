...
Later September 22 candidate update: build **1.0 (4)** from `51218f1` contains the verified panel geometry repair. Local archive/export, Apple validation, upload and processing have succeeded. Build 4 is assigned to **External Testers** and **Waiting for Review** after the required Beta App Review submission, with concrete What to Test instructions and automatic notification enabled for the existing tester. The quiet delivery follow-up will verify approval and external availability. Build 3 remains the available beta while review is pending. This does not alter the public listing draft.
