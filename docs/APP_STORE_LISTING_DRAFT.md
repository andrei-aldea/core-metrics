...
Later September 22 candidate update: build **1.0 (4)** from `51218f1` contains the verified panel geometry repair. Local archive/export and Apple validation pass. After earlier HTTP 504 failures, a scheduled retry of the same archive succeeded; the correct TestFlight record now shows build 4 **Processing**. The quiet delivery follow-up will complete external testing-group assignment after processing. Build 3 remains the available beta while that finishes. This does not alter the public listing draft.
