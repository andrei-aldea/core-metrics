...
Later September 22 candidate update: build **1.0 (4)** from `51218f1` contains the verified panel geometry repair. Local archive/export and Apple validation pass, but the upload failed with Apple's HTTP 504 gateway error and exit 70. Build 4 was absent after refreshing the correct TestFlight record. The preserved candidate has a quiet 30-minute delivery follow-up; build 3 remains the available beta. This does not alter the public listing draft.
