...
`CoreMetricsApp` owns one `MetricsStore` and one `PreferencesStore` as `@State`. It injects both into the status label/panel and Settings. It also owns a `LaunchAtLoginStore` for Settings and injects the copy writer into the panel. `MenuBarLabelView.task` starts sampling idempotently, including when the label is used as the Settings preview. Sampling continues while the panel is closed because the menu-bar value remains live.
...
`MenuBarLabelView` shares formatting, selection order, and the full spoken summary between the status item and Settings. It presents a single native `Text` containing an `AttributedString` with the existing system monospaced font and a locale-aware width reservation. The existing window-style `MenuBarExtra` owns the status item and panel. The text supplies “Core Metrics” as its accessibility label and the full metric summary as its accessibility value. Native text replaces the former capped bitmap presentation after a user-reported typography and visibility regression; see [ADR-015](DECISIONS.md#adr-015--restore-native-status-text).

`MenuBarLabelLayout` measures locale-specific digit, decimal-separator, and percent glyphs to calculate a frame reservation independent of current readings. The label holds this layout in state and refreshes it when the locale changes. Layout fixtures check text against the requested frame; the native host may adapt its font and width, so those fixtures do not guarantee fixed on-screen dimensions. There is no status image cache or app-imposed width cap. Menu-bar space remains controlled by macOS and must be checked in the running app, particularly for long selections on crowded or notched displays.
...
`CurrentReadingsFormatting` is a nonisolated formatter for full selected names and locale-aware values. The button reads current immutable snapshots synchronously on the main actor before formatting. A user-triggered button writes that text using `NSPasteboard`, reports success inline, and presents a native alert on failure. It does not read previous clipboard contents or retain a history. The OS owns the copied content after the write, including any enabled Universal Clipboard behavior. `MetricHelpView` explains existing definitions and update behavior using local, selectable, scrollable text. See [design](DESIGN.md) for accessibility validation constraints.
