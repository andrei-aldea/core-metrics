...
**Mode sizing, inter-stat spacing and preview attachment handling superseded by [ADR-021](#adr-021--share-native-sizing-across-four-display-modes).** The measured value columns, tabular digits and locale-aware percent-gap strategy remain current.
...
## ADR-021 — Share native sizing across four display modes

The user requested the same label/value size in Compact, category icons with values, and values without labels for every selection count. Use the shared 12-point `MenuBarLabelLayout.pointSize` for labels, separators, tabular-digit values and symbol configurations in all four modes. Compact changes only names to existing short codes. All modes separate complete slots with two spaces. One set of measured percentage/memory/storage columns per locale replaces the two font-size caches; retain ADR-020's percent-gap calculation, point alignment and fixed native status-item allocation.

Macs Fan Control is a visual sizing reference from the supplied screenshot and [vendor examples](https://crystalidea.com/macs-fan-control). The vendor page does not specify a font point size, and attempts to inspect the installed app timed out. The implementation matches the reference's apparent size with native text; it does not claim access to Macs Fan Control's internal font configuration. Apple's [menuBarFont(ofSize:)](https://developer.apple.com/documentation/appkit/nsfont/menubarfont(ofsize:)) documents the platform default when passed zero; the local probe returned 13 points for both that default and `systemFontSize`. The supplied reference's visible “71°C” glyphs measure 51 × 18 pixels at the screenshot's native resolution. Rendering the same text with the native system font at 2× produced 51 × 18 pixels at 12 points and 55 × 19 at 13 points, using the same brightness threshold. This supports 12 points as the visual match rather than the platform default.

Icon and Value prefixes each selected stat with its existing CPU, memory or storage SF Symbol, including repeated categories. Cache configured template images once using public [NSImage.SymbolConfiguration](https://developer.apple.com/documentation/appkit/nsimage/symbolconfiguration-swift.class) and [NSTextAttachment](https://developer.apple.com/documentation/appkit/nstextattachment). Center each symbol's bounds on the label font's cap height. Measure the same attributed prefix when reserving the native width. Settings converts the native title into interpolated SwiftUI Text runs, mapping font, kerning and attachment image/baseline offset explicitly. Values remain native text; neither mode rasterizes the whole label.

A zero-width left-to-right mark precedes every unlabeled slot. Without a strong direction anchor, Arabic percent direction marks can reorder neutral category symbols or shift subsequent columns across changing values. Preserve the localized value string and its own symbol order/marks while fixing the canonical CPU → Memory → Storage slot order. Full metric names and Unavailable remain in the shared accessibility summary and tooltip, independently of visible mode; copied readings retain full names.

All four modes support one to seven selected stats, and selection changes retain the chosen mode. Preserve the legacy `iconAndValue` raw value for Label and Value; the new icon mode uses `categoryIconAndValue`. Preserve `valueOnly`, `compact`, all stat identifiers and both legacy preference schemas. Default selection and mode remain CPU User / Label and Value. Native radio controls in Settings expose all four choices without fitting their labels into one segmented control.

A larger Compact allocation is intentional. Do not shrink, truncate, cap or hide readings to fit a crowded menu bar. Values Only, fewer selections, the full scrollable preview and the public Finder reopen-to-Settings route remain available. Validation and actual display-capacity limits belong in the latest project report.

## ADR-022 — Allow first login registration and expose public release links

**Date:** September 12, 2026.

The installed app reported `SMAppService.Status.notFound` and disabled Launch at Login before registration could be attempted. Apple's DTS guidance explains that this status can also mean the system has never seen the service. Treat it as eligible for an explicit attempt; keep unknown states disabled and let ServiceManagement validate the actual app. Never register automatically or persist a second setting. Preserve approval, cancellation, error and retry behavior. [Apple DTS](https://developer.apple.com/forums/thread/719862).

The companion Support and Privacy Policy pages now exist at verified HTTPS routes. Expose them with native `Link` controls, with the policy link always visible beside the Privacy sheet's Done button. Fixed URLs carry no readings or preferences and open the default browser without new app networking or entitlements. Public-policy approval and distribution signing remain publisher release requirements; links alone do not make a legal draft ready for submission.

## ADR-023 — Give native Settings a compact content width

**Date:** September 12, 2026.

The Settings scene opened at 900 points despite a smaller ideal width. Minimum-size hints alone did not enable native resizing. Explicitly enabling the public resize behavior changed the native flag and zoom control, but border-resize diagnostics still did not shrink the window on the reviewed beta runtime. Remove that override and use the Settings scene's documented content-size strategy with a 580-point content width, flexible height and the existing scrolling form. Keep native scene ownership, window restoration, commands and modal presentation.

The desktop regression checks the actual 580-point window, all four radio choices and the support/help/privacy controls. This verifies the chosen content-sized presentation; it does not claim to fix the beta's border-resize behavior. Long localized text, larger desktop text and other display configurations remain part of release validation. [Apple's scene sizing strategies](https://developer.apple.com/documentation/swiftui/windowresizability/automatic).

## ADR-024 — Keep the selection panel anchored to the status item's right edge

**Date:** September 19, 2026.

Selecting or deselecting a reading immediately changes the status item's allocation. Anchoring the popover to the entire status button makes the open panel follow that changing geometry. Apple documents that a popover automatically moves when its [positioning view changes location or size](https://developer.apple.com/documentation/appkit/nspopover/positioningrect).

The presenter owns a one-point-wide, nondrawing view at the button's right edge. Its [autoresizing mask](https://developer.apple.com/documentation/appkit/nsview/autoresizingmask-swift.property) keeps only the left margin flexible horizontally, preserving that anchor as the status item grows or shrinks to the left. The native popover positions against this view's bounds. The anchor is excluded from accessibility and returns `nil` from [hitTest](https://developer.apple.com/documentation/appkit/nsview/hittest(_:)), leaving the whole native status button interactive. This uses an owned subview, without inspecting AppKit's internal view hierarchy.

The September 22 RC desktop regression exposed a second timing step: the status button resizes before its backing window moves. The anchor's autoresizing notification can therefore position the popover against an intermediate screen location. While shown, the presenter owns a weakly captured task observing the backing window's public [`didMoveNotification`](https://developer.apple.com/documentation/appkit/nswindow/didmovenotification). If the button size changed, it reapplies the anchor after that move using [`show(relativeTo:of:preferredEdge:)`](https://developer.apple.com/documentation/appkit/nspopover/show(relativeto:of:preferrededge:)), which also updates an already-visible popover. Pure window movement keeps AppKit’s automatic tracking without another show call. Closing or destroying the presenter cancels the observation. This observes geometry only; it adds no application-focus observer or input monitor.

Selection persistence, immediate label updates and width recomputation remain unchanged. Keep native transient dismissal, focus, expanded-session coordination, keyboard Settings, and Escape. AppKit still controls placement when screens or available menu-bar space change. The regression compares actual panel frames while selecting and deselecting readings, verifies concurrent status-width changes, then checks dismissal, reopening and the button's rightmost click target. Validation results and limitations are recorded in the project report.


## ADR-025 — Remove the unused character-padding renderer
