...
The script checks macOS 27/Apple silicon, Xcode 27+, the macOS 27+ SDK and Swift 6+, then serially lists the project, resolves dependencies, builds Debug, runs unit tests, builds Release and analyzes. It lints source and packaged property lists, verifies the reviewed sandbox/privacy declarations and packaged metadata against resolved build settings, and checks that Release excludes DEBUG UI-test markers and unexpected nested bundles. It also checks that bundled files are readable, directories traversable and the main executable runnable by all users. Permission regression fixtures include the owner-only signature resource that Apple rejected during release validation. Hardened runtime must be enabled in both source app configurations and in resolved Release settings; a resolved Debug value of `NO` is explicitly reported as a development/test limitation. Project, source entitlement and privacy-manifest fingerprints must remain unchanged. Optional ad-hoc UI tests run afterward in separate DerivedData; their app's effective sandbox entitlement and bundle permissions are checked. Final working and staged diff checks run even after a failed build step.

Xcode commands run with `umask 022` inside an owner-only validation directory; raw logs and captured configuration remain owner-only. Do not apply a restrictive umask to Xcode's build process: a valid signature can still contain unreadable signature resources and fail Apple validation. Run `bash scripts/test-bundle-permissions.sh` for the focused regression fixtures, or `xcrun swift scripts/validate-artifacts.swift permissions '/path/to/Core Metrics.app'` to inspect an archived or expanded app without modifying it. The fixtures remove only their own temporary test directory; validation products and logs are retained.
...
For interactive work, select `Core Metrics` / `My Mac` and Run in Xcode. Stop that debug session before launching the command-line UI suite: if the debugger pauses on the suite’s termination signal, the retained development status item can obstruct test instances. The current local setup uses “Sign to Run Locally”; publisher signing is a separate release action. Keep identifiers, signing identities, teams and provisioning credentials unchanged unless explicitly authorized.
...
The two display-mode regressions select three categories and cycle all four modes in light/dark app appearances. Each mode must keep a stable native frame across 9%, 100%, and 9% CPU readings; captures record its status label and Settings preview. The tests require full accessibility context and restore Icon and Value with all three stats after relaunch. The seven-stat flow now preserves Values Only while selecting stats, checks a wider Compact allocation while Settings remains open, then restores Values Only for native hit testing and saved-selection startup. Full-size Compact may need more physical menu-bar room than the preceding 11-point revision.
...
| App Intents metadata extraction skipped | Xcode beta and the reviewed RC emit a build-tool warning when no AppIntents dependency exists. No documented supported opt-out was established for this target. Record it and recheck stable Xcode; do not add an unused framework or suppress warnings. |
...
## Production readiness and installed-copy checks

As of September 12, Apple accepts Xcode 27 RC (`27A266a`) and its macOS 27 RC SDK for App Store submissions. The RC was downloaded from Apple, independently checked with strict signature verification and Gatekeeper, and installed alongside beta 6. Existing license acceptance was valid and first-launch setup completed. Select the RC per invocation with `DEVELOPER_DIR=/Applications/Xcode.app/Contents/Developer`; the global Xcode selection and project configuration remain unchanged. Use the report's final accepted-toolchain results before distribution. See [APP_STORE.md](APP_STORE.md).

Launch at Login can initially report `notFound` even for an installed copy that has never registered. That state must allow the explicit toggle action. Enable through the actual installed Release, confirm the OS-backed success or approval message, reopen Settings to refresh, exercise removal/re-registration as requested, and verify after a real login. Do not infer failure solely from `notFound`, bypass macOS approval, install a helper, or register from a test fixture. If registration fails, retain the real status and review the error locally without publishing paths or signing details. Automated fixtures use a fake service that begins at `notFound`.

When a local app update is requested, stop this project's Xcode debug session before testing or replacement. Validate the latest source, build a locally signed Release in an isolated directory, verify its strict signature, effective sandbox-only entitlements, hardened runtime, packaged privacy and absence of DEBUG hooks, then back up the existing installed copy before replacing it. Check the installed executable against that exact validated artifact, launch it through Finder, and verify saved selections, all four modes and Launch at Login. Keep the previous copy recoverable. An ad-hoc local Release is not an App Store distribution artifact; it must never be uploaded as one.

For that local Release only, pass `CODE_SIGN_IDENTITY=- CODE_SIGNING_REQUIRED=YES CODE_SIGN_INJECT_BASE_ENTITLEMENTS=NO` to `xcodebuild`. The September 12 beta toolchain otherwise injected `com.apple.security.get-task-allow` into an ad-hoc Release despite the sandbox-only source entitlements. Verify the effective signature afterward; the installed local copy must have the runtime flag and exactly `com.apple.security.app-sandbox = true`. This command override does not change source signing or publisher configuration. Keep the normal debugger entitlement available for Debug/UI-test builds.

The September 12 desktop suite adds a native Settings content-width regression. It opens Settings through the public app-reopen route, requires the actual 580-point width, verifies all four display controls and the support/help/privacy actions remain inside the window, and captures the result. The ordinary light/dark flows check the public-policy link outside the scrolling text. These checks do not open external websites or change real login registration. Website HTTP checks and final installed-app registration are separate evidence.

The final `OiB8mN` RC validator passed all requested steps, including **71 unit declarations / 171 executions and all 12 UI tests together**, with no failures or skips. It retained five AppIntents build warnings; exported app output also retained layout/autoShortcut diagnostics, detailed in the report. The final installed RC-built local Release passed strict signature, hardened-runtime, sandbox-only entitlement, privacy and DEBUG-exclusion checks. Launch at Login stayed enabled after replacement, and all four modes were inspected before restoring the original selection. The publisher chose to keep local identity/signing; these checks do not create an uploadable App Store artifact.
