...
These are implemented test scenarios and validation requirements, not a claim that every run passed. Final results and remaining runtime coverage are in the [report](PROJECT_ANALYSIS_REPORT.md); distinguish the superseded image-label results from restored native-text checks. The native host exposes the combined app-name/metric-summary accessibility label through its `title`; it does not expose the separate accessibility value. Assert that summary independently of screenshot inspection of rendered abbreviations. Settings preview must expose the same summary. XCTest exposes the native Launch at Login control as a Switch and may expose segmented selection as NSNumber rather than String; account for those native representations when querying controls. Keep accessibility identifiers stable and update the test's application lookup only as part of an authorized identity change.
