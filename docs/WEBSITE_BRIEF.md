...
People choose **one to seven statistics** from a persistent selection panel. Choices stay in CPU → Memory → Storage order. The native text label uses normal system typography for names and separators, with monospaced, padded live values and locale-aware number formatting. The app reserves space for the selected configuration so changing values do not request a new width. Local runtime checks verified unchanged width and position across alternating short/long percentages, and the three-stat full-name and seven-stat Compact scenarios passed on the reviewed desktop. Keep claims scoped to that evidence; never promise every long selection fits beside a notch or other menu-bar items.
